...
do
	...
	do
		...
		for j in {1..65}	# range of IPs to send messages
		do
			$NC $DEST_IP.$j -u 514 -w 1 <<< "<$PRIORITY>`env LANG=us_US.UTF-8 date "+%b %d %H:%M:%S"` $SOURCE [$FACILITY.$LEVEL] service: $RANDOM_MESSAGE"
			#echo $NC $DEST_IP.$j -u 514 -w 1  "<$PRIORITY>`env LANG=us_US.UTF-8 date "+%b %d %H:%M:%S"` $SOURCE service: $RANDOM_MESSAGE"
		done
	done
	sleep $SLEEP_SECS
done
